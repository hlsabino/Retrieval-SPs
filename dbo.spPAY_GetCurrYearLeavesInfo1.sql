...
GO
CREATE PROCEDURE [dbo].[spPAY_GetCurrYearLeavesInfo1]
			...
			BEGIN			
				...
				--START :CURRENT DATERANGE LEAVES TAKEN
					--START: LOADING DATES FROM @MONTHTAB TABLE	   
				   	DECLARE @DATESCOUNT TABLE (SNO INT IDENTITY(1,1),ID INT ,DATE1 DATETIME,DAYNAME VARCHAR(50),WEEKNO INT,COUNT INT,NOOFDAYS DECIMAL(9,2),WKOFFCOUNT INT)
				...
				--START WEEKLYOFF COUNT
					...
					DECLARE @I INT,@J INT,@COLNAME VARCHAR(50)
					SET @STRQUERY=''	   
					...
					BEGIN
						...
						--update #EMPWEEKLYOFF set WK11='' where WK11='None'
					...
					BEGIN
						...
						EXEC sp_executesql @STRQUERY
						--EMPLOYEE MASTER
						IF((SELECT COUNT(*) FROM COM_CC50051 WHERE NODEID=@EmployeeID AND (ISNULL(WeeklyOff1,'')<>'' or ISNULL(WeeklyOff1,'None')<>'None') AND (ISNULL(WeeklyOff2,'')<>'' or ISNULL(WeeklyOff2,'None')<>'None'))>0)
						BEGIN
							SET @I=1
							SET @STRQUERY=''
							WHILE(@I<=5)
					...
					END
					--SELECT * FROM #EMPWEEKLYOFF
					SELECT * FROM #EMPWEEKLYOFF1
					--SELECT * FROM #WEEKLYOFF
					--SELECT * FROM #WEEKLYOFF1
...
--select * from @WEEKOFFCOUNT
				...
				--print @NoOfHolidayOP 
				--print @NoOfWkOffsOP 
				--print @CurrYearLeavesTakenOP 
				--print @EncahsedLeavesOP 
