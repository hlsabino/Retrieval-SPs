...
GO
CREATE PROCEDURE [dbo].[spPAY_RptGetAttendanceCalendar]
...
SELECT DCALPHA1,DCALPHA3,DCCCNID51,dcAlpha2,dcAlpha4,ID.STATUSID,dcAlpha35,dcAlpha37 INTO #T1 
...
WHERE ID.COSTCENTERID=40089 AND ID.STATUSID NOT IN (372,376) 
...
((ISDATE(DCALPHA1)=1 AND CONVERT(DATETIME,DCALPHA1)=CONVERT(DATETIME,@DT1))
OR
(ISDATE(DCALPHA3)=1 AND CONVERT(DATETIME,DCALPHA3)=CONVERT(DATETIME,@DT1))
))
BEGIN
	SET @StartTime=''
	SET @EndTime=''

	SELECT @StartTime=SUBSTRING(CONVERT(VARCHAR(8), CONVERT(DATETIME,dcAlpha2), 108),1,5),@EndTime=SUBSTRING(CONVERT(VARCHAR(8), CONVERT(DATETIME,dcAlpha4), 108),1,5) FROM #T1		WHERE DCCCNID51=@NODEID AND ((ISDATE(DCALPHA1)=1 AND CONVERT(DATETIME,DCALPHA1)=CONVERT(DATETIME,@DT1))
OR
(ISDATE(DCALPHA3)=1 AND CONVERT(DATETIME,DCALPHA3)=CONVERT(DATETIME,@DT1))
)
	...
	OR
	(ISDATE(DCALPHA3)=1 AND CONVERT(DATETIME,DCALPHA3)=CONVERT(DATETIME,@DT1))
	) AND @StartTime!=@EndTime AND STATUSID=369)
	BEGIN
	...
	OR
	(ISDATE(DCALPHA3)=1 AND CONVERT(DATETIME,DCALPHA3)=CONVERT(DATETIME,@DT1))
	) AND STATUSID=371 AND (ISNULL(dcAlpha35,'')<>'' OR ISNULL(dcAlpha37,'')<>'') )
	BEGIN
		UPDATE @TEMP SET DATYPE='R',StartTime=@StartTime,EndTime=@EndTime WHERE NODEID=@NODEID AND DADATE=@DT1
	END
	ELSE IF EXISTS(SELECT * FROM #T1 WITH(NOLOCK) WHERE DCCCNID51=@NODEID AND  
	((ISNULL(DCALPHA1,'')<>'' AND ISDATE(DCALPHA1)=1 AND CONVERT(DATETIME,DCALPHA1)=CONVERT(DATETIME,@DT1))
	OR
	(ISNULL(DCALPHA3,'')<>'' AND ISDATE(DCALPHA3)=1 AND CONVERT(DATETIME,DCALPHA3)=CONVERT(DATETIME,@DT1))
	) AND STATUSID=369)
	BEGIN
