...
GO
CREATE PROCEDURE [dbo].[spCom_GetAddressByID]
	@CostCenterID [int] = 0,
	@AddressByID [nvarchar](max) = null,
	@UserID [int],
	@LangID [int] = 1
...
SET NOCOUNT ON		
		...
		'+@CustomQuery1+'
		WHERE c.FeatureID='+convert(nvarchar(300),@CostCenterID)+' and  c.AddressID in ('+convert(nvarchar(300),@AddressByID) +')'
		PRINT @sql
