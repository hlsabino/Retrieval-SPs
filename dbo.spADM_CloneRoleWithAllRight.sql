...
GO
CREATE PROCEDURE [dbo].[spADM_CloneRoleWithAllRight]
	@NewRoleID [int],
	@NewRoleName [nvarchar](50),
	@EditRoleID [int],
	@EditRoleName [nvarchar](50),
	@LangID [int] = 1,
...
BEGIN TRY    
  ...
  SELECT CostCenterID,nodeid,0,@NewRoleID,UserID,@LoginUserName,convert(float,Getdate()) 
  from ADM_Assign with(nolock) where RoleID=@EditRoleID
  --VPT
  INSERT [dbo].[ADM_DocPrintLayoutsMap] ([DocPrintLayoutID], [UserID], [RoleID], [GroupID], [CreatedBy], [CreatedDate], [PrintOtherVPT], [PrintContinue], [CCNID2], [BasedOn]) 
  SELECT  [DocPrintLayoutID], [UserID], @NewRoleID, [GroupID], @LoginUserName, convert(float,Getdate()) , [PrintOtherVPT], [PrintContinue], [CCNID2], [BasedOn] 
  from ADM_DocPrintLayoutsMap with(nolock) where RoleID=@EditRoleID
  --Reports
  INSERT INTO ADM_ReportsUserMap(UserID,RoleID,GroupID,ReportID,CreatedBy,CreatedDate,ActionType)
  Select UserID,@NewRoleID,GroupID,ReportID,@LoginUserName, convert(float,Getdate()),ActionType from 
  ADM_ReportsUserMap with(nolock)
...
INSERT INTO [COM_NotifTemplateUserMap]([NotificationID],[GroupID],[RoleID],[UserID],[CreatedBy],[CreatedDate],[BasedOnDimension],[BasedOnField])
   select  [NotificationID] ,[GroupID]    ,@NewRoleID   ,UserID   ,@LoginUserName    ,convert(float,Getdate()) ,[BasedOnDimension],[BasedOnField] from [COM_NotifTemplateUserMap]
   with(nolock) 	where RoleID=@EditRoleID
		
		
--NotificationDeviceID		
INSERT INTO  [Com_UserNotificationDeviceID]([UserID],[RoleID],[CCID],[NodeID],[DeviceID],[CreatedOn],[UpdatedOn])
Select UserID      ,@NewRoleID   ,[CCID] ,[NodeID] ,[DeviceID],convert(float,Getdate()) ,convert(float,Getdate())
from [Com_UserNotificationDeviceID]     with(nolock) 	where  RoleID=@EditRoleID
--view
...
Select  [CostCenterID] ,[FeatureID] ,[ViewName]  ,[ResourceID]  ,[SearchFilter] ,@NewRoleID  ,UserID    
		...
		from [ADM_GridView]	with(nolock) 	where  RoleID=@EditRoleID

---Quik view 

	INSERT INTO ADM_QuickViewDefnUserMap(QID,ShowCCID,GroupID,RoleID,UserID)
	SELECT QID,ShowCCID,GroupID,@NewRoleID,UserID from ADM_QuickViewDefnUserMap with(nolock)
	where  RoleID=@EditRoleID


	  --Dasboard Copy
  INSERT INTO ADM_DashBoardUserRoleMap(DashBoardID,GroupID,RoleID,UserID,CompanyGUID,GUID,CreatedBy,CreatedDate,IsDefault)  
  SELECT DashBoardID,GroupID,@NewRoleID,UserID,CompanyGUID,newid(),GUID,convert(float,Getdate()),IsDefault  
  FROM ADM_DashBoardUserRoleMap   with(nolock)   where  RoleID=@EditRoleID


		
  --SELECT * FROM ADM_ReportsUserMap  where    RoleID>9
   --SELECT CostCenterID,nodeid,0,RoleID,UserID,convert(float,Getdate())  from ADM_Assign with(nolock) where RoleID=505
 
 --SELECT * FROM ADM_Features WHERE FeatureID IN (49,100,105,1)

  --SELECT * FROM [ADM_DocPrintLayoutsMap]
--  --Dasboard Copy
--  INSERT INTO ADM_DashBoardUserRoleMap(DashBoardID,GroupID,RoleID,UserID,CompanyGUID,GUID,CreatedBy,CreatedDate,IsDefault)  
