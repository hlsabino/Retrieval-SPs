...
GO
CREATE PROCEDURE [dbo].[spINV_MoveAboveProduct]
	@ProductID [bigint],
...
BEGIN TRY  
		...
		BEGIN   
					...
					UPDATE INV_Product  SET lft=lft+@Width+1 WHERE lft>=@Selectedlft and rgt<@lft  
					UPDATE INV_Product  SET rgt=rgt+@Width+1 WHERE lft>=@Selectedlft and rgt<@lft  

					--ADJUST LEFT,RIGHTS AND DEPTH OF THE PRODUCT AND CHILDS
					UPDATE INV_Product  SET rgt=@Selectedlft-@lft+rgt,lft=@Selectedlft-@lft+lft					
		...
		BEGIN  
					...
					--ADJUST LEFT,RIGHTS AND DEPTH OF THE PRODUCT AND CHILDS
					UPDATE INV_Product  SET  rgt=@Selectedrgt-1+rgt-@rgt,lft=@Selectedrgt-1+lft-@rgt
					WHERE ProductID  in (SELECT ID from @Temp)  
