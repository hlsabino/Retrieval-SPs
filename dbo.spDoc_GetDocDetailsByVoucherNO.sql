...
GO
CREATE PROCEDURE [dbo].[spDoc_GetDocDetailsByVoucherNO]
...
SET NOCOUNT ON;
		...
		IF (@voucherno='' or @CostCenterID=0)
		BEGIN
			RAISERROR('-100',16,1)
		END
		if exists(select COstcenterid from adm_documentTypes WITH(NOLOCK) where COstcenterid=@CostCenterID and isinventory=1)
		begin
			select distinct DocPrefix Prefix,DocNumber,CostCenterID CCID,l.ResourceData,DisplayName
			...
			where VoucherNo=@voucherno and COstcenterid=@CostCenterID
		end
		else
		begin
			select distinct DocPrefix Prefix,DocNumber,CostCenterID CCID,l.ResourceData,DisplayName
