...
GO
CREATE PROCEDURE [dbo].[spACC_MoveAfterAccount]
	@AccountID [bigint],
...
BEGIN TRY  
		...
		BEGIN   
					...
					UPDATE ACC_Accounts  SET lft=lft+@Width+1 WHERE lft>@Selectedrgt and rgt<@lft  
					UPDATE ACC_Accounts  SET rgt=rgt+@Width+1 WHERE lft>@Selectedrgt and rgt<@lft  

					--ADJUST LEFT,RIGHTS AND DEPTH OF THE PRODUCT AND CHILDS
					UPDATE ACC_Accounts  SET rgt=@Selectedrgt+1-@lft+rgt,lft=@Selectedrgt+1-@lft+lft					
		...
		BEGIN  
					...
					--ADJUST LEFT,RIGHTS AND DEPTH OF THE PRODUCT AND CHILDS
					UPDATE ACC_Accounts  SET  rgt=@Width-@lft+rgt,lft=@Width-@lft+lft
					WHERE AccountID in (SELECT ID from @Temp)  
