...
GO
CREATE PROCEDURE [dbo].[spRPT_StockValuationTagWise]
	@TagID [nvarchar](max),
...
SET NOCOUNT ON;  
	...
	DECLARE @SQL NVARCHAR(MAX),@RecQty FLOAT,@ProductID BIGINT,@TagSQL NVARCHAR(MAX),@ProductList NVARCHAR(MAX),
	...
	BEGIN
		...
		BEGIN
			...
			--	print @ProductID
				...
				--TO GET BALANCE DATA
				EXEC [spRPT_AvgRate] 0,@ProductID,@SubTagSQL,@ToDate,@ToDate,@IncludeUpPostedDocs,@DefValuation,0,'',0,@BalQty OUTPUT,@AvgRate OUTPUT,@BalValue OUTPUT
