...
GO
CREATE PROCEDURE [dbo].[spCOM_MoveAfterCostCenter]
	@CostCenterId [int],
	@NodeID [bigint],
...
BEGIN TRY  
		...
		BEGIN   
					...
					UPDATE '+@Table+'  SET lft=lft+@Width+1 WHERE lft>@Selectedrgt and rgt<@lft  
					UPDATE '+@Table+'  SET rgt=rgt+@Width+1 WHERE lft>@Selectedrgt and rgt<@lft  

					--ADJUST LEFT,RIGHTS AND DEPTH OF THE PRODUCT AND CHILDS
					UPDATE '+@Table+'  SET rgt=@Selectedrgt+1-@lft+rgt,lft=@Selectedrgt+1-@lft+lft					
		...
		BEGIN  
					...
					--ADJUST LEFT,RIGHTS AND DEPTH OF THE PRODUCT AND CHILDS
					UPDATE '+@Table+'  SET  rgt=@Width-@lft+rgt,lft=@Width-@lft+lft
					WHERE NodeID in (SELECT ID from @Temp)  
