...
GO
CREATE PROCEDURE [dbo].[spACC_MoveMultipleAccount]
	@SelectedNodeID [bigint],
	@AccountXML [nvarchar](max),
	@RoleID [bigint],
	@LangID [int] = 1
...
--SET NOCOUNT ON; 
		...
		--Declaration Section  
  		DECLARE @HasAccess BIT,@AccountID BIGINT,@return_value int,@XML xml,@I int,@Cnt int

		--Check for manadatory paramters  
		IF(@AccountXML='' OR @SelectedNodeID=0)
		BEGIN  
			RAISERROR('-100',16,1)   
		END
		...
		CREATE TABLE #tblList(ID int identity(1,1),ACCOUNTID BIGINT)  


		--Read XML data into temporary table only to delete records
		INSERT INTO #tblList
		SELECT X.value('@AccountID','BIGINT')
		...
		BEGIN
			...
			SELECT @AccountID=ACCOUNTID FROM #tblList WITH(nolock) WHERE ID=@I  			
			SET @I=@I+1
			EXEC	@return_value = [dbo].[spACC_MoveAccount]
					@AccountID = @AccountID,
