...
GO
CREATE PROCEDURE [dbo].[spRPT_MoveMultipleReport]
...
SET NOCOUNT ON; 
		...
		--Declaration Section  
  		DECLARE @HasAccess BIT,@ReportID BIGINT,@return_value int,@XML xml,@I int,@Cnt int

		--Check for manadatory paramters  
		IF(@ReportXML='' OR @SelectedNodeID=0)
		BEGIN  
			RAISERROR('-100',16,1)   
		END
		...
		CREATE TABLE #tblList(ID int identity(1,1),ReportID BIGINT)  


		--Read XML data into temporary table only to delete records
		INSERT INTO #tblList
		SELECT X.value('@ReportID','int')
		...
		BEGIN
			...
			SELECT @ReportID=ReportID FROM #tblList WITH(nolock) WHERE ID=@I  			
			SET @I=@I+1
			EXEC	@return_value = [dbo].[spRPT_MoveReport]
					@ReportID = @ReportID,
