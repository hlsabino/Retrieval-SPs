...
GO
CREATE PROCEDURE [dbo].[spCRM_MoveLead]
	@LeadID [bigint],
...
SET NOCOUNT ON; 
		...
		IF(@LeadID=0 OR @SelectedNodeID=0)
		BEGIN  
			RAISERROR('-100',16,1)   
		END
		...
		--GETTING THE CHILDS OF Lead 
			...
			BEGIN  
					...
					BEGIN  
						...
						UPDATE CRM_Leads  SET rgt=rgt+@Width+1 WHERE rgt>@Selectedlft and rgt<@lft  
						UPDATE CRM_Leads  SET lft=lft+@Width+1 WHERE lft>@Selectedlft and lft<@lft  

						--ADJUST LEFT,RIGHTS AND DEPTH OF THE Lead AND CHILDS
						UPDATE CRM_Leads  SET  Depth=Depth -@Depth+@SelectedDepth +1, rgt=rgt- @lft+@Selectedlft+1,lft=lft- @lft+@Selectedlft+1  
						WHERE LeadID in (SELECT ID FROM @Temp)  

						--SET PARENT AS SELECTED NODE IF IT IS GROUP
						UPDATE CRM_Leads  SET ParentID=@SelectedNodeID  
						WHERE LeadID=@LeadID  
					...
					BEGIN   
						--ADJUST LEFT AND RIGHTS OF THE TREE BETWEEN SELECTED NODE AND Lead
						UPDATE CRM_Leads  SET rgt=rgt+@Width+1 WHERE rgt>@Selectedlft+1 and rgt<@lft  
						UPDATE CRM_Leads  SET lft=lft+@Width+1 WHERE lft>@Selectedlft and lft<@lft  

						--ADJUST LEFT,RIGHTS AND DEPTH OF THE Lead AND CHILDS
						UPDATE CRM_Leads  SET Depth=Depth -@Depth+@SelectedDepth,rgt=rgt- @lft+@Selectedrgt+1,lft=lft- @lft+@Selectedrgt+1  
						WHERE LeadID in (SELECT ID FROM @Temp)  
			...
			BEGIN  
					...
					BEGIN  
						--ADJUST LEFT AND RIGHTS OF THE TREE BETWEEN SELECTED NODE AND Lead
						UPDATE CRM_Leads  SET rgt=rgt-@Width-1 WHERE rgt>@rgt and rgt<=@Selectedlft  
						UPDATE CRM_Leads  SET lft=lft-@Width-1 WHERE lft>@rgt and lft<=@Selectedlft  

						--ADJUST LEFT,RIGHTS AND DEPTH OF THE Lead AND CHILDS
						UPDATE CRM_Leads  SET  Depth=Depth -@Depth+@SelectedDepth +1, rgt=rgt+(@Selectedlft-@Width)-@lft,lft=lft+(@Selectedlft-@Width)-@lft  
						WHERE LeadID in (SELECT ID FROM @Temp)  
					...
					BEGIN   
						--ADJUST LEFT AND RIGHTS OF THE TREE BETWEEN SELECTED NODE AND Lead
						UPDATE CRM_Leads  SET rgt=rgt-@Width-1 WHERE rgt>@rgt and rgt<=@Selectedlft+1  
						UPDATE CRM_Leads  SET lft=lft-@Width-1 WHERE lft>@rgt and lft<=@Selectedlft  

						--ADJUST LEFT,RIGHTS AND DEPTH OF THE Asset AND CHILDS
						UPDATE CRM_Leads  SET  Depth=Depth -@Depth+@SelectedDepth , rgt=rgt+(@Selectedlft-@Width)-@lft+1,lft=lft+(@Selectedlft-@Width)-@lft+1  
						WHERE LeadID in (SELECT ID from @Temp)  
