...
GO
CREATE PROCEDURE [dbo].[spREN_MoveAfterUnit]
	@UnitID [bigint],
...
BEGIN TRY  
	...
	--User access check 
	SET @HasAccess=dbo.fnCOM_HasAccess(@RoleID,93,110)
	...
	BEGIN   
		...
		UPDATE REN_Units  SET lft=lft+@Width+1 WHERE lft>@Selectedrgt and rgt<@lft  
		UPDATE REN_Units  SET rgt=rgt+@Width+1 WHERE lft>@Selectedrgt and rgt<@lft  

		--ADJUST LEFT,RIGHTS AND DEPTH OF THE PRODUCT AND CHILDS
		UPDATE REN_Units  SET rgt=@Selectedrgt+1-@lft+rgt,lft=@Selectedrgt+1-@lft+lft					
	...
	BEGIN  
		...
		--ADJUST LEFT,RIGHTS AND DEPTH OF THE PRODUCT AND CHILDS
		UPDATE REN_Units  SET  rgt=@Width-@lft+rgt,lft=@Width-@lft+lft
		WHERE UnitID  in (SELECT ID from @Temp)  
