...
GO
CREATE PROCEDURE [dbo].[spPRD_MoveMultipleBOM]
...
--SET NOCOUNT ON; 
		...
		--Declaration Section  
  		DECLARE @HasAccess BIT,@BOMID BIGINT,@return_value int,@XML xml,@I int,@Cnt int

		--Check for manadatory paramters  
		IF(@BOMXML='' OR @SelectedNodeID=0)
		BEGIN  
			RAISERROR('-100',16,1)   
		END
		...
		CREATE TABLE #tblList(ID int identity(1,1),BOMID BIGINT)  


		--Read XML data into temporary table only to delete records
		INSERT INTO #tblList
		SELECT X.value('@BOMID','BIGINT')
		...
		BEGIN
			...
			SELECT @BOMID=BOMID FROM #tblList WITH(nolock) WHERE ID=@I  			
			SET @I=@I+1
			EXEC	@return_value = [dbo].spPRD_MoveBOM
					@BOMID = @BOMID,
