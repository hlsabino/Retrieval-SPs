...
GO
CREATE PROCEDURE [dbo].[spCOM_GetActivityAssignedUsers]
	@CostCenterID [bigint],
	@NODEID [bigint],
	@SUBCostCenterID [bigint] = 0,
	@SUBNodeID [bigint] = 0
...
BEGIN
	...
	--EMAIL
		...
		BEGIN
			    ...
			    SELECT UserID FROM CRM_Assignment WITH(NOLOCK) WHERE CCID='+CONVERT(NVARCHAR,@CostCenterID)+' and CCNODEID='+CONVERT(NVARCHAR,@NODEID)+' AND ISFROMACTIVITY='+CONVERT(NVARCHAR,@SUBNodeID)+'
			    ...
			    WHERE CCID='+CONVERT(NVARCHAR,@CostCenterID)+' and CCNODEID='+CONVERT(NVARCHAR,@NODEID)+' AND ISFROMACTIVITY='+CONVERT(NVARCHAR,@SUBNodeID)+'
