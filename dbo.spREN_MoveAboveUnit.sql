...
GO
CREATE PROCEDURE [dbo].[spREN_MoveAboveUnit]
	@UnitID [bigint],
...
BEGIN TRY  
	...
	--User access check 
	SET @HasAccess=dbo.fnCOM_HasAccess(@RoleID,93,111)
	...
	BEGIN   
		...
		UPDATE REN_Units  SET lft=lft+@Width+1 WHERE lft>=@Selectedlft and rgt<@lft  
		UPDATE REN_Units  SET rgt=rgt+@Width+1 WHERE lft>=@Selectedlft and rgt<@lft  

		--ADJUST LEFT,RIGHTS AND DEPTH OF THE PRODUCT AND CHILDS
		UPDATE REN_Units  SET rgt=@Selectedlft-@lft+rgt,lft=@Selectedlft-@lft+lft					
	...
	BEGIN  
		...
		--ADJUST LEFT AND RIGHTS OF THE TREE BETWEEN SELECTED NODE AND PRODUCT
		UPDATE REN_Units  SET rgt=rgt-@Width-1 WHERE rgt>@rgt and rgt<=@Selectedrgt  
		UPDATE REN_Units  SET lft=lft-@Width-1 WHERE lft>@rgt and lft<=@Selectedrgt  

		set @Width=@Selectedrgt-@Width
		
		--ADJUST LEFT,RIGHTS AND DEPTH OF THE PRODUCT AND CHILDS
		UPDATE REN_Units  SET  rgt=@Selectedrgt-1+rgt-@rgt,lft=@Selectedrgt-1+lft-@rgt
		WHERE UnitID  in (SELECT ID from @Temp)  
