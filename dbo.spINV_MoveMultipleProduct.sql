...
GO
CREATE PROCEDURE [dbo].[spINV_MoveMultipleProduct]
...
SET NOCOUNT ON; 
		...
		--Declaration Section  
  		DECLARE @HasAccess BIT,@ProductID BIGINT,@return_value int,@XML xml,@I int,@Cnt int

		--Check for manadatory paramters  
		IF(@ProductXML='' OR @SelectedNodeID=0)
		BEGIN  
			RAISERROR('-100',16,1)   
		END
		...
		CREATE TABLE #tblList(ID int identity(1,1),ProductID BIGINT)  
		...
		BEGIN
			...
			SELECT @ProductID=ProductID FROM #tblList WITH(nolock) WHERE ID=@I  			
			SET @I=@I+1
			EXEC	@return_value = [dbo].[spINV_MoveProduct]
					@ProductID = @ProductID,
