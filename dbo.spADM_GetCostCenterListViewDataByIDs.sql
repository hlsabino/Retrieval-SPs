...
GO
CREATE PROCEDURE [dbo].[spADM_GetCostCenterListViewDataByIDs]
	@CostCenterID [int] = 0,
	@ListViewTypeID [int],
...
SET NOCOUNT ON
			...
			SELECT @Table=TableName,@Primarycol=PrimaryKey FROM ADM_Features  WITH(NOLOCK) WHERE FeatureID=@CostCenterID
			...
			ELSE if(@Primarycol is null or @Primarycol='')
