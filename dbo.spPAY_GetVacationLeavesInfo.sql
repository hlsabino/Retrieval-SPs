...
GO
CREATE PROCEDURE [dbo].[spPAY_GetVacationLeavesInfo]
	...
	@langid [int] = 1,
	@RptMode [int] = 0,
	@VacTaken [float] OUTPUT,
	@EncashedDays [float] OUTPUT
...
IF ((SELECT COUNT(ID.DocID) FROM INV_DOCDETAILS ID WITH(NOLOCK),COM_DOCTEXTDATA TD WITH(NOLOCK),COM_DOCCCDATA CC WITH(NOLOCK)
...
BEGIN
	...
	AND ID.INVDOCDETAILSID=TD.INVDOCDETAILSID AND ID.INVDOCDETAILSID=CC.INVDOCDETAILSID AND TD.INVDOCDETAILSID=CC.INVDOCDETAILSID AND CC.DCCCNID53=@GRADE 	
END
--END: CHECKING GRADE
...
BEGIN			
		...
		,0,ISNULL(dcAlpha16,''),CONVERT(FLOAT,dcAlpha14),CONVERT(DATETIME,dcAlpha1),CONVERT(FLOAT,dcAlpha10)  
		FROM COM_DocTextData TD with(nolock),INV_DOCDETAILS ID with(nolock), COM_DocCCData DC with(nolock) 
		WHERE  TD.tCostCenterID=40072 AND ID.STATUSID NOT IN (372,376) AND LEN(dcAlpha2)<30 AND  LEN(dcAlpha3)<30 AND ISDATE(dcAlpha2)=1 AND  ISDATE(dcAlpha3)=1 
		AND TD.InvDocDetailsID=DC.InvDocDetailsID AND	ID.InvDocDetailsID=TD.InvDocDetailsID AND DC.DCCCNID51=@EmployeeID  
		And (
			     CONVERT(DATETIME,dcAlpha2) between CONVERT(DATETIME,@FromDate) and CONVERT(DATETIME,@ToDate)
				 or 
				 CONVERT(DATETIME,dcAlpha3) between CONVERT(DATETIME,@FromDate) and CONVERT(DATETIME,@ToDate)
				 or CONVERT(DATETIME,@FromDate) between CONVERT(DATETIME,dcAlpha2) and CONVERT(DATETIME,dcAlpha3)
				 or CONVERT(DATETIME,@ToDate) between CONVERT(DATETIME,dcAlpha2) and CONVERT(DATETIME,dcAlpha3)
				 ) 
				 order by ID.Docdate
...
IF(@RptMode=0)
BEGIN
	SELECT *,@MWKOFFS MWeeklyoff,@MHDAYS MHolidays From @DATESAPPLIEDCOUNT  WHERE IsEncashDays<>'Yes'
END
ELSE
BEGIN

	SELECT @VacTaken=NOOFDAYS From @DATESAPPLIEDCOUNT WHERE  IsEncashDays<>'Yes'
	SELECT @EncashedDays=NOOFDAYS From @DATESAPPLIEDCOUNT WHERE  IsEncashDays='Yes'
	--select @VacTaken as VacTaken,@EncashedDays as EncashDays
END
