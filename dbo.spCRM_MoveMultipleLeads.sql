...
GO
CREATE PROCEDURE [dbo].[spCRM_MoveMultipleLeads]
...
--SET NOCOUNT ON; 
		...
		--Declaration Section  
  		DECLARE @HasAccess BIT,@LeadID BIGINT,@return_value int,@XML xml,@I int,@Cnt int

		--Check for manadatory paramters  
		IF(@LeadXML='' OR @SelectedNodeID=0)
		BEGIN  
			RAISERROR('-100',16,1)   
		END

		--User acces check
		SET @HasAccess=dbo.fnCOM_HasAccess(@RoleID,86,5)
		...
		CREATE TABLE #tblList(ID int identity(1,1),LeadID BIGINT)  


		--Read XML data into temporary table only to delete records
		INSERT INTO #tblList
		SELECT X.value('@LeadID','BIGINT')
		...
		BEGIN
			...
			SELECT @LeadID=LeadID FROM #tblList WITH(nolock) WHERE ID=@I  			
			SET @I=@I+1
			EXEC	@return_value = [dbo].[spCRM_MoveLead]
					@LeadID = @LeadID,
